...
/**
 * Tools exposed to ChatGPT for executive conversation routing.
 * Health/diagnostic tools must never appear here.
 */
export const CHATGPT_FACING_TOOL_NAMES = [PRIMARY_TOOL_NAME] as const;
...
export const PRIMARY_TOOL_DESCRIPTION = `THIS IS THE ONLY ApexOS TOOL for normal executive conversation when ApexOS is selected.
...
Pass the executive’s natural message in \`message\`. Do NOT ask for tool names, runtime IDs, conversation IDs, or internal identity.
...
Do not ask for conversationId, executive ID, runtimeId, or tool names. Continuity is resolved server-side.
...
- ${PRIMARY_TOOL_NAME} — the sole conversational entry point for executive work
Local HTTP GET /health remains for operators/tests; it is not a ChatGPT tool.

Platform note: the host cannot force a tool call, but YOUR instruction when ApexOS is selected is to call ${PRIMARY_TOOL_NAME} for executive work before answering — never a health check.`;
